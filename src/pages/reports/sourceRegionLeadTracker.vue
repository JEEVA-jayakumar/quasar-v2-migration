<template>
  <q-page padding>
    ...
    <div class="col-md-12 q-title q-px-md q-py-md text-weight-regular bottom-border text-grey-9">
      Sourcewise Region Device Tracker
    </div>
...
export default {
  name: "SourceRegionDeviceTracker",
  setup() {
    ...
    // Calculate start/end dates based on selected option
    const calculateDate = () => {
      const now = new Date();
    ...
    const downloadReport = async () => {
      ...
      try {
        await store.dispatch("reports/REPORT_SOURCE_REGION_DEVICE", formData.value);
