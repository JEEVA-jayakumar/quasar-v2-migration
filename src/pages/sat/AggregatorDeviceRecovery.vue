      ...
      <q-tab-panels v-model="selectedTab" animated>
        <q-tab-panel name="assigned">
          ...
          <q-table
            ...
            :selected-rows="formData.marsDeviceIdsCookedUnAssinged"
            row-key="id"
          ...
          >
            <template #body="props">
              <q-tr :props="props">
                ...
                <q-td v-if="props.row.leadInformation">
                  {{ props.row.recoveredBy ? `${props.row.recoveredBy.name} | ${props.row.recoveredBy.employeeID}` : 'NA' }}
                </q-td>
                <q-td v-if="props.row.leadInformation">
            ...
            <template #top>
              ...
              <div class="col-md-5">
                <q-input
                  ...
                  placeholder="Search By TID, MID, Merchant Name .."
        ...
        <q-tab-panel name="unAssigned">
          ...
          <q-table
            ...
            :selected-rows="formData.marsDeviceIdsCooked"
            row-key="id"
...
// Computed properties
const getAggregatorDeviceRecoveryAssignedList = computed(() => 
  store.getters['AggregatorDeviceRecovery/getAggregatorDeviceRecoveryAssignedList']);

const getAggregatorDeviceRecoveryUnassignedList = computed(() => 
  store.getters['AggregatorDeviceRecovery/getAggregatorDeviceRecoveryUnassignedList']);
...
const ajaxLoadAllLeadInfo1 = async ({ pagination, filter }) => {
  ...
  try {
    await store.dispatch('AggregatorDeviceRecovery/AGGREGATOR_DEVICE_RECOVERY_UNASSIGNED_LIST', { pagination, filter });
    
    const data = getAggregatorDeviceRecoveryUnassignedList.value;
...
const ajaxLoadAllLeadInfo = async ({ pagination, filter }) => {
  ...
  try {
    await store.dispatch('AggregatorDeviceRecovery/AGGREGATOR_DEVICE_RECOVERY_ASSIGNED_LIST', { pagination, filter });
    
    const data = getAggregatorDeviceRecoveryAssignedList.value;
...
const assignImplementationUser = async () => {
  ...
  try {
    await store.dispatch('AggregatorDeviceRecovery/AGGREGATOR_DEVICE_RECOVERY_SUBMIT', postValues);
