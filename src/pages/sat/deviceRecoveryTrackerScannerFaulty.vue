    ...
    <div>
      ...
      <div class="row bottom-border q-px-md q-py-md items-center">
        <div class="col-md-8 q-title text-weight-regular text-grey-9">
          Device Recovery Tracker - Faulty Inventory
        </div>
      ...
      <!-- Table -->
      <q-table
        :rows="getDeviceFaultyScannedItems"
        :columns="columnData"
        ...
        color="light-blue"
      >
        <!-- Action Column (Optional Remove button) -->
...
import { useBarcodeScanner } from "vue-barcode-scanner"; // Vue 3 compatible scanner
...
const getDeviceFaultyScannedItems = store.getters["SatDeviceTrackerScanner/getDeviceFaultyScannedItems"];
...
function onBarcodeScanned(barcode) {
  ...
  REACTIVE_FAULTY_SCANNED_DEVICE_DATA(tempTableData.value);
...
function removeScannedItem(item) {
  const updatedItems = getDeviceFaultyScannedItems.filter(i => i.serial_number !== item.serial_number);
  REACTIVE_FAULTY_SCANNED_DEVICE_DATA(updatedItems);
}
</script>
