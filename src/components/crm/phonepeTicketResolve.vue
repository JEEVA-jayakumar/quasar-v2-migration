...
const props = defineProps({
  resolveData: { type: Object, required: true },
  propToggleResolve: { type: Boolean, required: true }
})

const emit = defineEmits(['closeResolve'])
...
/* ============================
   Reactive State
============================ */
const toggleModal = ref(props.propToggleResolve)
...
/* ============================
   Watchers
============================ */
watch(() => props.propToggleResolve, val => toggleModal.value = val)
...
/* ============================
   Vuex Actions
============================ */
const { FETCH_ISSUE_DATAS, FETCH_SUB_ISSUE_DATAS, FETCH_REMARKS_ISSUE_DATAS } =
  store._modulesNamespaceMap['crmAction/'].context.actions
const { ASSIGN_TO_COMPLETE } = store._modulesNamespaceMap['phonePeCrm/'].context.actions
...
const fnResolve = async (request) => {
  $q.dialog({
    ...
    const payload = {
      ...
      resolutionRemarks: formData.remarks
    }

    try {
      await ASSIGN_TO_COMPLETE(payload)
