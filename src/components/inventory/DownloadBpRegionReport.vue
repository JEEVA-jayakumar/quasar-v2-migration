...
export default defineComponent({
  ...
  emits: ['emitfnshowBpRegionReport'],

  setup(props, { emit }) {
    const store = useStore()

    const toggleModel = ref(props.propBpRegionReport)
    ...
    const emitfnshowBpRegionReport = () => {
      emit('emitfnshowBpRegionReport')
    }
    ...
    const downloadDeviceBpRegion = async () => {
      const params = {
      ...
      try {
        await store.dispatch(
          'DownloadInventoryBPRegion/FETCH_INVENTORY_BP_REGIONS_DATAS',
          params
        )
...
<style scoped>
/* kept minimal – no functional change */
</style>
