...
const props = defineProps({
  propPhonepeCompletedTicket: { type: Boolean, required: true }
})
const emit = defineEmits(['emitfnshowPhonepeCrmComplete'])
...
/* ---------------- STATE ---------------- */
const toggleModel = ref(props.propPhonepeCompletedTicket)
...
/* ---------------- METHODS ---------------- */
function emitfnshowPhonepeCrmComplete() {
  emit('emitfnshowPhonepeCrmComplete')
}
...
async function downloadHistory() {
  ...
  try {
    await store.dispatch(
      'DownloadPhonepeCrmCallbackReport/FETCH_PHONEPE_CRM_COMPLETED_CALLBACK_REPORT',
      params
    )
