...
const props = defineProps({
  propMasterTrackerList: Boolean
});
const emit = defineEmits(["emitfnshowMasterTrackerList"]);
...
// Reactive modal & form
const toggleModal = ref(props.propMasterTrackerList);
...
watch(() => props.propMasterTrackerList, (val) => {
...
// Emit cancel
function emitfnshowMasterTrackerList() {
  emit("emitfnshowMasterTrackerList");
}

// Download Master Tracker List
function downloadMasterTrackerList() {
  ...
  store.dispatch("DownloadMasterTrackerData/MASTER_TRACKER_LIST_ALL_DATAS", params)
    .then(() => {
      $q.loading.hide();
      $q.notify({
        color: "positive",
    ...
    .catch((error) => {
      ...
      else if (error.status === 404) msg = "Please choose another date";
