...
const props = defineProps({
  propAggrMasterTrackerList: Boolean
});
const emit = defineEmits(["emitfnshowAggrMasterTrackerList"]);
...
// Reactive state
const toggleModal = ref(props.propAggrMasterTrackerList);
...
// Watch prop changes
watch(() => props.propAggrMasterTrackerList, (val) => {
  toggleModal.value = val;
});

// Computed: disable download button if dates not selected
const submitDisabled = computed(() => {
  return !formData.fromDate || !formData.toDate;
});

// Emit cancel
function emitfnshowAggrMasterTrackerList() {
  emit("emitfnshowAggrMasterTrackerList");
}

// Download Tracker File
function downloadAggrMasterTrackerList() {
  ...
  store.dispatch("DownloadMasterTrackerData/AGGREGATOR_MASTER_TRACKER_LIST_ALL_DATAS", params)
    .then(() => {
      $q.loading.hide();
      $q.notify({
        color: "positive",
