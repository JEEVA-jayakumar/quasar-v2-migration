...
function downloadLostDatas() {
  ...
  store.dispatch("DownloadLostOrStolenAllDatas/FETCH_LOST_OR_STOLEN_ALL_DATAS", params)
    .then(() => {
      $q.loading.hide();
      $q.notify({
        color: "positive",
    ...
    .catch((error) => {
      ...
      else if (error.status === 403 || error.status === 404) msg = "Please choose another date";
