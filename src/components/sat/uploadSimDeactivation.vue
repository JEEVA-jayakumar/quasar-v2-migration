    ...
    <q-dialog v-model="toggleModel" persistent :maximized="false">
      <q-card style="min-width: 50vw; padding: 10px;">
        ...
        <q-card-actions align="right" class="col-md-12">
          <q-btn outline color="dark" label="Cancel" @click="emitfnshowSimDeActivationList" />
          <q-btn
            class="common-btn"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const toggleModel = ref(props.propSimDeactivation);
    ...
    // Vuex Actions
    const { SIM_DEACTIVE_UPLOAD_REGIONAL_INVENTORY } = mapActions(
      "SatRegionalInventory",
      ["SIM_DEACTIVE_UPLOAD_REGIONAL_INVENTORY"]
    );

    // Watch prop for open/close
    watch(
    ...
    // Cancel / Close
    const emitfnshowSimDeActivationList = () => {
      formData.sim = null;
    ...
    const uploadSimDeactivation = async () => {
      ...
      try {
        ...
        const response = await SIM_DEACTIVE_UPLOAD_REGIONAL_INVENTORY({
          file: assumeFormData,
