...
const InventoryCentral = {
  ...
  actions: {
    ...
    /* AGGREGATORS API */
    FETCH_AGGREGATORS_CENTRAL_INVENTORY_DASHBOARD_COUNT({commit}) {
      return api
    ...
    FETCH_ALL_PHONE_PE_INVENTORY_DEVICES_TYPES_WITH_COUNT_DATA({commit}) {
    ...
    /*END >> Module>> all lead validations*/
    /*Aggregators API */
    FEED_AGGREGATORS_FAULTY_FINAL_SUBMIT(_, request) {
      return api.put(
    ...
    FEED_ALLOCATE_SO_TO_SO(_, request) {
      return api.post(
        "assign-sat-to-so/1",
        request
      );
    ...
    FEED_AGGREGATORS_ALLOCATE_SO_TO_SO(_, request) {
      return api.post(
        "assign-sat-to-so/2",
        request
      );
    ...
    async REPORT_HITACHI_ONBOARDING_MERCHANTS() {
      return await api
        .get("download-InventorySummaryInfoMenu/1", {
          responseType: "arraybuffer"
        })
    ...
    async PRINT_MERCHANT_DETAILS(_, request) {
      const response = await api.get(
    ...
    async PHONEPE_PAPER_ROLL_PDF(_, request) {
      const response = await api.get(
