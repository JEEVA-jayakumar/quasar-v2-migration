<template>
  <q-page class="q-pa-md">
    ...
    <div class="row items-center text-weight-regular bottom-border text-grey-9 q-pa-md">
      <div class="col-md-8 q-title">Aggregator Faulty Device</div>
    ...
    <showAggregatorsRepairConfirmation
...
onMounted(() => {
  if (!VueBarcodeScanner.installed) VueBarcodeScanner.install();
...
// Load aggregator device types
const AggregatorsDeviceType = async () => {
  try {
...
const onBarcodeScanned = (barcode) => {
  ...
  if (!selectedDevice.deviceSerialNumbers.includes(barcode)) {
    store.dispatch("VerifyDevice/AGGREAGATORS_DEVICE_VERIFICATION_IN_CENTRAL_FOR_ACTIVE", {
